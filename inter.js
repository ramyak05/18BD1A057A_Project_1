...
app.get('/getHospitalData',middleware.checkToken,(req,res)=>{    
    ...
    db.collection('hospital').find({},{projection: {"_id":0}}).toArray().then(result=> res.json(result));  
});

//fetch ventilators data
app.get('/getVentilatorData',middleware.checkToken,(req,res)=>{    
    console.log("Fetching Ventilators Data");
    db.collection('ventilators').find({},{projection: {"_id":0}}).toArray().then(result=> res.json(result));  
});
...
app.post('/searchStatus',middleware.checkToken,(req,res)=>{
    ...
    console.log(status);
    db.collection('ventilators').find({status:status},{projection: {"_id":0}}).toArray().then(result=> res.json(result));  
});
...
app.post('/searchHospitalName',middleware.checkToken,(req,res)=>{
    ...
    console.log(name);
    db.collection('ventilators').find({'Name': new RegExp(name,'i')},{projection: {"_id":0}}).toArray().then(result=> res.json(result));   
});
...
app.post('/searchHospital',middleware.checkToken,(req,res)=>{
    var name=req.query.name;
    console.log(name);
    db.collection('hospital').find({'Name': new RegExp(name,'i')},{projection: {"_id":0}}).toArray().then(result=> res.json(result));   
});
